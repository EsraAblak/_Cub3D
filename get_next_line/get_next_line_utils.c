...
/*   Updated: 2023/06/05 16:21:58 by eablak           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_g_strjoin(char *s1, char *s2)
{
	...
	united = (char *)malloc(sizeof(char) * ft_g_strlen(s1) + ft_g_strlen(s2)
			+ 1);
	if (united == NULL)
		return (0);
	i = -1;
	...
	while (s2[j] != '\0')
		united[i++] = s2[j++];
	united[ft_g_strlen(s1) + ft_g_strlen(s2)] = '\0';
	free(s1);
	return (united);
}
...
char	*ft_take_rest(char *all_line)
{
	...
	rest = (char *)malloc(sizeof(char) * ft_g_strlen(all_line) - i + 1);
	if (!rest)
		return (0);
	i++;
	while (all_line[i] != '\0')
